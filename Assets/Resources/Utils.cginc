...
//random samples on the hemisphere
float3 GGX_Sample_wh(float u, float v, float3 N, float roughness )
{
	// 这是cosine Weight采样
	float phi = v * 2.0 * PI; 
	...
	float cos2Theta = (1 - u) / ((alpha*alpha - 1)*u + 1);
	...
	float3 wh = TangentX * H.x + TangentY * H.y + N * H.z;
	float coswh = dot(wh, N);
