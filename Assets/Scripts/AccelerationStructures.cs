...
public class AccelerationStructures
{
    ...
    public static ComputeBuffer MatBuffer;
    public static ComputeBuffer MatIdxBuffer;
    ...
    {
        Vector3 v0 = new Vector3(0, 2, 0);
        Vector3 v1 = new Vector3(2f, 1, 0);
        Vector3 v2 = new Vector3(0, 1, 2f);
        ...
        ray.direction = new Vector3(1, 2f, 1);
        float t = 0;
        Vector3 b;
        ray_triangle_intersection(ray, ivm, out t, out b);
    }


    //taken from "gpu-based techniques for global illumination effects"
    static bool ray_triangle_intersection(in Ray ray, Matrix4x4 tri, out float t, out Vector3 b)
    {
        b = Vector3.zero;
        bool ris = false;
        Vector3 qPrime = tri.GetRow(0) + tri.GetRow(1) + tri.GetRow(2);
        Vector3 q = qPrime / Vector3.Dot(qPrime, qPrime);
        t = (Vector3.Dot(q, q) - Vector3.Dot(ray.origin, q)) / Vector3.Dot(ray.direction, q);
        if (t > 0.001)
        {
            Vector3 p = ray.origin + t * ray.direction;

            b.x = Vector3.Dot(tri.GetRow(0), p);
            b.y = Vector3.Dot(tri.GetRow(1), p);
            b.z = Vector3.Dot(tri.GetRow(2), p);
    ...
    public static void BuildTriangleList()
    {
        ...
        List<int> matIdx_list = new List<int>();
        List<Material> mat_set = new List<Material>();
        ...
        foreach (MeshRenderer r in renderers)
        {
            ...
            for (int t = 0; t < num_tris; ++t)
            {
                ...
                if (!mat_set.Contains(r.sharedMaterial))
                {
                    mat_set.Add(r.sharedMaterial);
                }

                int matIdx = mat_set.IndexOf(r.sharedMaterial);
                matIdx_list.Add(matIdx);
        ...
        if (triangle_list.Count > 0)
        {
            ...
            TriangleBuffer.SetData(triangle_list.ToArray());

            TrisVertexBuffer = new ComputeBuffer(vertex_list.Count, 64);
            TrisVertexBuffer.SetData(vertex_list.ToArray());
            ...
            // 处理材质索引缓冲
            MatIdxBuffer = new ComputeBuffer(matIdx_list.Count, 4);
            MatIdxBuffer.SetData(matIdx_list.ToArray());

            // 转换材质
            List<PTMaterial> PTMats = ConvertMaterialData(mat_set);

            MatBuffer = new ComputeBuffer(PTMats.Count, Marshal.SizeOf(typeof(PTMaterial)));
            MatBuffer.SetData(PTMats.ToArray());
        }
    }

    public static List<PTMaterial> ConvertMaterialData(List<Material> mat_set)
    {
        List<PTMaterial> result = new List<PTMaterial>();

        foreach (var mat in mat_set)
        {
            PTMaterial ptMat = new PTMaterial();


            if (mat.HasProperty("_EmissionColor"))
            {
                Color clr = mat.GetColor("_EmissionColor");
                ptMat.Albedo = new Vector3(clr.r, clr.g, clr.b);
                if (clr.r > 0.1f)
                {
                    ptMat.isLight = 1;
                }
            ...
            if (ptMat.isLight == 0)
            {
                ...
                {
                    ptMat.metallic = mat.GetFloat("_Metallic");
                }

                if (mat.HasProperty("_Glossiness"))
                {
                    ptMat.smoothness = mat.GetFloat("_Glossiness");
                }
            }
